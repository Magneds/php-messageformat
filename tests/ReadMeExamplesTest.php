...
class ReadMeExamplesTest extends TestCase {
    ...
    public function testFormatMessage() {
        $this->assertEquals('Hello universe', MessageFormatter::formatMessage('en', 'Hello {audience}', ['audience' => 'universe']));
    }

	public function testFormat() {
        $es = new MessageFormatter('es-ES', 'Por el pequeño precio de {price, number, currency} puedes comprar apps.');

        $this->assertEquals('Por el pequeño precio de 0,99 € puedes comprar apps.', $es->format(['price' => 0.99]));
	}
    ...
    public function testGetPattern() {
        ...
        $this->assertEquals('Welcome back {name}, you have {count, plural, =0{no unread messages} one{one unread message} other{# unread messages}}', $en->getPattern());
        $this->assertEquals('Welcome back {0}, you have {1, plural, =0{no unread messages} one{one unread message} other{# unread messages}}', $en->getPattern(true));
    ...
    public function testParse() {
        ...
        $this->assertEquals(['animal' => 'kat', 'action' => 'krabt', 'result' => 'krullen', 'target' => 'trap'], $parsed);
    }
